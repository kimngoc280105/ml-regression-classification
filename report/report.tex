...
\renewcommand{\theHsubsection}{\thepart.\arabic{section}.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{\thepart.\arabic{section}.\arabic{subsection}.\arabic{subsubsection}}
\makeatother
...
\midrule
3 & 23120062 & Trần Kim Ngọc & Phụ trách Regression: EDA, tiền xử lý, phần nâng cao (bao gồm GPR, Robust Reg., phân tích Bias-Variance thực nghiệm) và tổng hợp báo cáo. & 100\% \\
\midrule
...
\begin{table}[H]
  ...
  \small
  \begin{tabularx}{\textwidth}{P{0.17\textwidth}P{0.35\textwidth}Y}
    \toprule
    ...
    Cubic Spline
      & \begin{tabular}[t]{@{}l@{}}
          $\phi_j(x) = \mathrm{SplineBasis}_j(x)$\\
          (piecewise cubic, knot quantiles)
        \end{tabular}
      & Trơn bậc 3, liên tục đạo hàm bậc 1 và 2 tại knots \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Dự án triển khai các nhóm mô hình phân lớp chính và bonus sau; bảng này đồng thời kiểm tra mức độ bao phủ yêu cầu của đề bài.
...
Phần phân tích so sánh và kỹ năng nghiên cứu & Được tách thành một phần riêng của báo cáo, gồm kết nối hồi quy--phân lớp, bảng so sánh toàn diện, sensitivity/robustness, convergence và khả năng tái hiện. \\
...
Bộ \adult có 32,561 dòng và 15 cột, trong đó biến mục tiêu là \texttt{income}. Các đặc trưng gồm nhóm số như \texttt{age}, \texttt{education.num}, \texttt{capital.gain}, \texttt{capital.loss}, \texttt{hours.per.week} và nhóm phân loại như \texttt{workclass}, \texttt{occupation}, \texttt{relationship}, \texttt{race}, \texttt{sex}. Dữ liệu chính được giữ ở dạng gốc trong \codepath{data/adult.csv}; notebook thực hiện tiền xử lý trong bộ nhớ để tránh thay đổi file nguồn.
...
Lớp dương $>50K$ chỉ chiếm khoảng một phần tư dữ liệu, vì vậy phần đánh giá dùng thêm Precision, Recall, F1, AUC, AP, Balanced Accuracy và confusion matrix thay vì chỉ dựa trên Accuracy.

\section{Tiền xử lý dữ liệu}
\label{sec:cls_preprocessing}

\subsection{Xử lý giá trị thiếu}

Bộ Adult dùng ký hiệu \texttt{?} cho một số giá trị thiếu ở các cột phân loại. Notebook thay \texttt{?} trong \texttt{workclass}, \texttt{occupation} và \texttt{native.country} bằng mode của từng cột, giữ nguyên kích thước dữ liệu và tránh loại bỏ nhiều mẫu thuộc lớp thiểu số.

\subsection{Mã hóa đặc trưng phân loại}

Nhãn mục tiêu được ánh xạ $\leq 50K \rightarrow 0$ và $>50K \rightarrow 1$. Các đặc trưng phân loại chính như \texttt{workclass}, \texttt{occupation}, \texttt{relationship}, \texttt{sex} và \texttt{race} được label-encode để dùng trong các mô hình tự cài đặt từ đầu.

\subsection{Feature Engineering}

Notebook tạo thêm đặc trưng \texttt{capital\_net = capital.gain - capital.loss}. Ma trận đặc trưng chính sau xử lý gồm 12 đặc trưng, kết hợp nhóm số gốc, \texttt{capital\_net} và các biến phân loại đã mã hóa.

\subsection{Chuẩn hóa và phân chia dữ liệu}

Dữ liệu được chia theo tỷ lệ 70/10/20 cho train/validation/test, tương ứng 22,792/3,256/6,513 mẫu. \texttt{StandardScaler} chỉ được fit trên tập train rồi transform validation và test để tránh rò rỉ dữ liệu.

\section{Nền tảng lý thuyết}
\label{sec:cls_theory}

\subsection{Bài toán phân lớp}

Cho tập dữ liệu $\mathcal{D}=\{(\mathbf{x}_n,t_n)\}_{n=1}^{N}$ với
$\mathbf{x}_n\in\mathbb{R}^{D}$ và $t_n$ là nhãn rời rạc. Trong bài toán nhị phân,
$t_n\in\{0,1\}$; trong bài toán đa lớp, $t_n\in\{1,\ldots,K\}$. Mục tiêu của mô
hình phân lớp là học hàm quyết định hoặc xác suất hậu nghiệm:
\begin{equation}
  P(t=k\mid \mathbf{x}), \qquad
  \hat{t}=\arg\max_{k}P(t=k\mid \mathbf{x}).
  \label{eq:cls_posterior}
\end{equation}
Khác với hồi quy, đầu ra của phân lớp không phải giá trị liên tục mà là nhãn hoặc
xác suất thuộc lớp. Vì vậy hàm mất mát thường xuất phát từ likelihood của phân
phối Bernoulli/Categorical thay vì Gaussian noise.

\subsection{Logistic Regression và Bernoulli likelihood}

Với vector đã thêm bias $\tilde{\mathbf{x}}=[1,\mathbf{x}^{T}]^{T}$, Logistic
Regression nhị phân mô hình hóa:
\begin{equation}
  y_n=P(t_n=1\mid \mathbf{x}_n)=\sigma(a_n),
  \qquad
  a_n=\boldsymbol{\theta}^{T}\tilde{\mathbf{x}}_n,
  \qquad
  \sigma(a)=\frac{1}{1+\exp(-a)}.
  \label{eq:cls_logistic_model}
\end{equation}
Vì $t_n\sim\operatorname{Bernoulli}(y_n)$, negative log-likelihood chính là
...
Gradient có dạng:
\begin{equation}
  \nabla E(\boldsymbol{\theta})=\tilde{X}^{T}(\mathbf{y}-\mathbf{t}),
  \label{eq:cls_logistic_gradient}
\end{equation}
nên sai lệch xác suất dự đoán trừ nhãn thật đóng vai trò tương tự residual trong
hồi quy tuyến tính.

\subsection{Softmax Regression cho phân lớp đa lớp}

Với $K$ lớp, mỗi lớp có vector trọng số $\mathbf{w}_k$. Logit và xác suất softmax:
\begin{equation}
  a_k=\mathbf{w}_k^{T}\tilde{\mathbf{x}},
  \qquad
  y_k=P(t=k\mid \mathbf{x})
  =\frac{\exp(a_k)}{\sum_{j=1}^{K}\exp(a_j)}.
  \label{eq:cls_softmax}
\end{equation}
Với nhãn one-hot $t_{nk}$, loss đa lớp là:
\begin{equation}
  E(W)=-\sum_{n=1}^{N}\sum_{k=1}^{K}t_{nk}\ln y_{nk}.
  \label{eq:cls_softmax_loss}
\end{equation}
Jacobian của softmax:
\begin{equation}
  \frac{\partial y_j}{\partial a_k}=y_j(\delta_{jk}-y_k),
  \qquad
  J=\operatorname{diag}(\mathbf{y})-\mathbf{y}\mathbf{y}^{T}.
  \label{eq:cls_softmax_jacobian}
\end{equation}
Điều này cho thấy xác suất các lớp cạnh tranh với nhau: tăng logit của một lớp
sẽ làm giảm xác suất tương đối của các lớp còn lại.

\subsection{Ước lượng tham số: GD, Newton--Raphson và IRLS}

Gradient Descent cập nhật tham số theo:
\begin{equation}
  \boldsymbol{\theta}^{(r+1)}
  =\boldsymbol{\theta}^{(r)}-\eta\nabla E(\boldsymbol{\theta}^{(r)}).
  \label{eq:cls_gd_update}
\end{equation}
Với Logistic Regression nhị phân, Hessian:
\begin{equation}
  H=\nabla^{2}E(\boldsymbol{\theta})=\tilde{X}^{T}R\tilde{X},
  \qquad
  R=\operatorname{diag}\left(y_n(1-y_n)\right).
  \label{eq:cls_hessian}
\end{equation}
Newton--Raphson dùng thông tin độ cong:
\begin{equation}
  \boldsymbol{\theta}^{(r+1)}
  =\boldsymbol{\theta}^{(r)}-H^{-1}\nabla E(\boldsymbol{\theta}^{(r)}).
  \label{eq:cls_newton}
\end{equation}
Dạng IRLS viết lại mỗi bước Newton như một bài toán weighted least squares:
\begin{equation}
  \boldsymbol{\theta}^{(r+1)}
  =(\tilde{X}^{T}R\tilde{X})^{-1}\tilde{X}^{T}R\mathbf{z},
  \qquad
  \mathbf{z}=\tilde{X}\boldsymbol{\theta}^{(r)}+R^{-1}(\mathbf{t}-\mathbf{y}).
  \label{eq:cls_irls}
\end{equation}
Newton thường cần ít vòng lặp hơn GD nhưng mỗi vòng lặp tốn chi phí cao hơn do
phải lập và giải hệ liên quan đến Hessian.

\subsection{Mô hình sinh: LDA, QDA và Gaussian Naive Bayes}

Các mô hình sinh học $p(\mathbf{x}\mid t=k)$ và prior $\pi_k=P(t=k)$, sau đó dùng
Bayes:
\begin{equation}
  P(t=k\mid \mathbf{x})
  =\frac{p(\mathbf{x}\mid t=k)\pi_k}
  {\sum_{j=1}^{K}p(\mathbf{x}\mid t=j)\pi_j}.
  \label{eq:cls_bayes_rule}
\end{equation}
LDA giả định Gaussian theo lớp với covariance chung:
\begin{equation}
  p(\mathbf{x}\mid t=k)=\mathcal{N}(\mathbf{x}\mid\boldsymbol{\mu}_k,\Sigma),
  \label{eq:cls_lda_assumption}
\end{equation}
nên boundary là tuyến tính. QDA dùng covariance riêng từng lớp:
\begin{equation}
  p(\mathbf{x}\mid t=k)=\mathcal{N}(\mathbf{x}\mid\boldsymbol{\mu}_k,\Sigma_k),
  \label{eq:cls_qda_assumption}
\end{equation}
nên boundary có thể là bậc hai. Gaussian Naive Bayes giả định các feature độc
lập có điều kiện theo lớp:
\begin{equation}
  p(\mathbf{x}\mid t=k)=\prod_{j=1}^{D}\mathcal{N}(x_j\mid\mu_{kj},\sigma_{kj}^{2}).
  \label{eq:cls_gnb_assumption}
\end{equation}

\subsection{Fisher ratio và phép chiếu LDA}

Fisher ratio đo khả năng tách lớp của từng feature trong bài toán nhị phân:
\begin{equation}
  J_j=\frac{(\mu_{1j}-\mu_{0j})^{2}}{\sigma_{1j}^{2}+\sigma_{0j}^{2}}.
  \label{eq:cls_fisher_ratio}
\end{equation}
Với LDA nhị phân, vector chiếu tối ưu theo Fisher criterion:
\begin{equation}
  \mathbf{w}\propto S_W^{-1}(\boldsymbol{\mu}_1-\boldsymbol{\mu}_0),
  \label{eq:cls_lda_projection}
\end{equation}
trong đó $S_W$ là within-class scatter matrix. Hướng chiếu này làm tăng khoảng
cách giữa hai trung bình lớp sau chiếu và giảm độ phân tán trong từng lớp.

\subsection{Regularization, class weighting và bias--variance}

Regularization thêm penalty vào loss để kiểm soát độ phức tạp:
\begin{align}
  E_{L2}(\boldsymbol{\theta})
  &=E(\boldsymbol{\theta})+\frac{\lambda}{2N}\|\boldsymbol{\theta}\|_2^2, \\
  E_{L1}(\boldsymbol{\theta})
  &=E(\boldsymbol{\theta})+\frac{\lambda}{N}\|\boldsymbol{\theta}\|_1.
  \label{eq:cls_regularization}
\end{align}
L2 làm co hệ số và giảm variance; L1 khuyến khích nghiệm thưa nên hữu ích cho
feature selection. Với dữ liệu mất cân bằng lớp, class-weighted cross-entropy:
\begin{equation}
  E=-\sum_{n=1}^{N}\sum_{k=1}^{K}c_k t_{nk}\ln y_{nk},
  \qquad
  c_k\propto\frac{N}{N_k}.
  \label{eq:cls_class_weight}
\end{equation}
trả chi phí lớn hơn cho lỗi ở lớp thiểu số. Đây là cách dịch chuyển decision
boundary theo mục tiêu đánh đổi Precision--Recall thay vì chỉ tối ưu Accuracy.

\section{Các mô hình phân lớp}

\subsection{Logistic Regression và các phương pháp tối ưu}

\subsubsection{Logistic Regression nhị phân và Gradient Descent}
...
\subsubsection{Newton--Raphson / IRLS}
...
\subsubsection{Softmax đa lớp, Jacobian và Hessian}
...
\subsubsection{One-vs-rest, one-vs-one và softmax trực tiếp}
...
\subsection{LDA/QDA và Fisher ratio}

\subsubsection{LDA và QDA}
...
\subsubsection{VC dimension và SRM}
...
Nếu dùng threshold mặc định và cần cân bằng giữa Accuracy, F1, tốc độ hội tụ và khả năng giải thích, LR-Newton là lựa chọn mạnh nhất cho phần classification. Nếu mục tiêu nghiệp vụ ưu tiên phát hiện nhiều mẫu $>50K$ hơn, LR-Weighted phù hợp hơn vì tăng Recall lớp dương rõ rệt.

\subsection{Ảnh hưởng của mất cân bằng lớp}

Adult là bài toán mất cân bằng lớp, nên mô hình có Accuracy cao chưa chắc hữu ích nếu bỏ sót quá nhiều mẫu dương. Điều này giải thích vì sao nhóm ưu tiên F1 lớp dương, Recall, PR/AP và Balanced Accuracy khi phân tích kết quả.

\subsection{Hạn chế và đề xuất cải tiến}

Các mô hình tuyến tính dễ giải thích nhưng khó nắm bắt interaction phi tuyến giữa nghề nghiệp, giờ làm, tuổi và học vấn. Các hướng cải tiến hợp lý gồm threshold tuning, calibration hậu xử lý, thêm interaction feature có kiểm soát và thử các mô hình phi tuyến có regularization tốt hơn.

\newpage
\begin{samepage}
\section{Kết luận}

Tất cả yêu cầu classification chính và bonus đã được bao phủ. Điểm quan trọng nhất không chỉ là LR-Newton có Accuracy/F1 cao, mà là mỗi mô hình thể hiện một trade-off khác nhau. LR-Newton là lựa chọn cân bằng nhất khi dùng threshold mặc định: Accuracy 0.8267, F1 0.5529. LR-Weighted là lựa chọn tốt nhất khi ưu tiên lớp thiểu số: Recall 0.7666, F1 0.6091. LDA/QDA/GNB có giá trị phân tích theo hướng generative; LDA tốt hơn GNB/QDA theo F1 trên Adult, còn QDA có AUC cao nhất nhưng cần threshold tuning. Kernel LR chứng minh lợi ích của phi tuyến trên XOR-like pattern, còn Laplace cung cấp thông tin bất định quanh decision boundary.
...
\newpage
\part{Phân tích So sánh và Kỹ năng Nghiên cứu}

\phantomsection
\section*{Tổng quan Phần 4}
\addcontentsline{toc}{section}{Tổng quan Phần 4}

Phần này không tính điểm riêng như một bài toán độc lập, nhưng là tiêu chí chất lượng bắt buộc của báo cáo. Nội dung được tách khỏi hai phần Regression và Classification để thể hiện khả năng tư duy phản biện, tổng hợp kết quả và kết nối kiến thức giữa hai bài toán.
...
\begin{table}[H]
\centering
\caption{Bảng so sánh toàn diện các nhóm mô hình chính trong hai phần.}
\label{tab:reg_cls_comparison}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.25}
\resizebox{\textwidth}{!}{%
\begin{tabular}{P{0.15\textwidth}P{0.20\textwidth}P{0.20\textwidth}P{0.20\textwidth}P{0.20\textwidth}}
\toprule
\textbf{Tiêu chí} & \textbf{Linear Reg.} & \textbf{Ridge/Lasso} & \textbf{Logistic Reg.} & \textbf{LDA/QDA} \\
\midrule
Giả thiết phân phối
& Sai số Gaussian, phương sai đồng nhất trong OLS.
& Giữ giả thiết Gaussian, thêm prior/penalty lên hệ số.
& Bernoulli hoặc Categorical likelihood cho nhãn rời rạc.
& Gaussian theo từng lớp; LDA dùng covariance chung, QDA dùng covariance riêng. \\
\midrule
Nghiệm dạng đóng
& Có nghiệm normal equation.
& Ridge có nghiệm đóng; Lasso thường cần solver lặp do L1 không trơn.
& Không có nghiệm đóng; dùng GD, Newton hoặc IRLS.
& Có nghiệm từ ước lượng mean, covariance và prior của từng lớp. \\
\midrule
Độ phức tạp huấn luyện
& Khoảng $O(ND^2+D^3)$ nếu giải trực tiếp.
& Ridge tương tự Linear Reg.; Lasso phụ thuộc số vòng lặp của solver.
& GD: $O(ND)$ mỗi epoch; Newton: $O(ND^2+D^3)$ mỗi epoch.
& LDA khoảng $O(ND^2+D^3)$; QDA tốn hơn do covariance riêng từng lớp. \\
\midrule
Khả năng giải thích
& Cao, hệ số tuyến tính đọc trực tiếp được.
& Ridge ổn định hệ số; Lasso hỗ trợ chọn đặc trưng nhờ sparsity.
& Giải thích qua log-odds và xác suất dự đoán.
& LDA giải thích tốt qua Fisher ratio/projection; QDA khó hơn vì boundary bậc hai. \\
\midrule
Nhạy với outlier
& Nhạy với outlier và heteroscedasticity.
& Giảm variance nhưng vẫn chịu ảnh hưởng điểm ngoại lai.
& Nhạy với điểm xa boundary hoặc nhãn nhiễu.
& Nhạy vì mean/covariance có thể bị kéo bởi outlier. \\
\midrule
Hiệu năng thực nghiệm
& OLS: RMSE $\$66{,}948$, $R^2=0.6561$.
& Ridge: RMSE $\$66{,}936$; Lasso: RMSE $\$66{,}974$.
& LR-Newton: Accuracy 0.8267, F1 0.5529; LR-Weighted: F1 0.6091.
& LDA: F1 0.5012; QDA: F1 0.4139, AUC 0.8526. \\
\bottomrule
\end{tabular}}
\end{table}
...
Với hồi quy, nhóm kiểm tra ba khía cạnh: thay đổi tỷ lệ train/test split
(60/40, 70/30, 80/20), thêm Gaussian noise vào đặc trưng đã chuẩn hóa, và
làm hỏng ngẫu nhiên 10\%, 20\%, 30\% giá trị đặc trưng rồi so sánh mean,
median và KNN imputation.

\figfull{figures/regression_outputs/sensitivity_split.png}{Boxplot RMSE của các mô hình hồi quy theo tỷ lệ train/test split và seed.}

Kết quả sensitivity cho thấy \textbf{Cubic Spline} ổn định nhất trong nhóm hồi
quy với Std RMSE khoảng $\$902$ qua 15 lần chạy, kế đến là Kernel Ridge
Regression với Std RMSE khoảng $\$943$. Polynomial degree 2 kém ổn định nhất
vì RMSE dao động rất mạnh giữa các split.

\figfull{figures/regression_outputs/noise_injection.png}{RMSE và mức suy giảm hiệu năng khi thêm Gaussian noise vào đặc trưng hồi quy.}

Khi thêm noise, nhóm mô hình tuyến tính có regularization ổn định hơn các mô
hình phi tuyến. \textbf{Elastic Net} có mean degradation thấp nhất
($45.33\%$), tiếp theo là Lasso ($46.45\%$) và Ridge ($48.39\%$). Các mô hình
phi tuyến như Spline, Polynomial và Gaussian RBF dễ khuếch đại nhiễu hơn vì
không gian đặc trưng phức tạp hơn.

\figfull{figures/regression_outputs/feature_corruption.png}{Mức suy giảm RMSE khi làm hỏng đặc trưng và áp dụng các chiến lược imputation.}

Với feature corruption, \textbf{KNN imputation} vượt trội rõ rệt: mean
degradation khoảng $5.66\%$, thấp hơn nhiều so với mean imputation
($40.12\%$) và median imputation ($49.22\%$). Điều này phù hợp với dữ liệu
California Housing vì các mẫu gần nhau trong không gian đặc trưng thường có
thông tin địa lý và dân cư tương đồng.

\figfull{figures/regression_outputs/convergence_curves.png}{Loss curves của các thuật toán lặp trong phần hồi quy.}

Về hội tụ, Bayesian Ridge đạt điểm evidence tốt nhất sau khoảng 9 iterations,
Mini-batch GD cần khoảng 100 epochs để ổn định, còn Lasso và Elastic Net bằng
coordinate descent hội tụ sau khoảng 80 iterations.
...
\bibitem{uci}
D. Dua and C. Graff, "UCI Machine Learning Repository," 2019. [Online]. Available: \url{https://archive.ics.uci.edu}
